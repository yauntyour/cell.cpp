    ...
    namespace box
    {
        ...
        static bool credential_leak(std::string_view call)
        {
            ...
            for (auto &t : vault_terms)
            {
                for (const char *sep : {"/", "\\"})
                {
                    if (!root_s.empty() && lower.find(std::string(root_s) + sep + std::string(t)) != std::string::npos)
                        return true;
                    if (!root_name.empty() && lower.find(std::string(root_name) + sep + std::string(t)) != std::string::npos)
                        return true;
                }
            }
            return false;
